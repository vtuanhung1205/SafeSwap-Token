...
export declare class VoidProvider extends Provider {
    ...
    executeTransactionWithRequestType(_txnBytes: string, _signatureScheme: SignatureScheme, _signature: string, _pubkey: string, _requestType: ExecuteTransactionRequestType): Promise<SuiExecuteTransactionResponse>;
    ...
    getEventsByTransaction(_digest: TransactionDigest, _count: number): Promise<SuiEvents>;
    getEventsByModule(_package: string, _module: string, _count: number, _startTime: number, _endTime: number): Promise<SuiEvents>;
    getEventsByMoveEventStructName(_moveEventStructName: string, _count: number, _startTime: number, _endTime: number): Promise<SuiEvents>;
    getEventsBySender(_sender: SuiAddress, _count: number, _startTime: number, _endTime: number): Promise<SuiEvents>;
    getEventsByRecipient(_recipient: ObjectOwner, _count: number, _startTime: number, _endTime: number): Promise<SuiEvents>;
    getEventsByObject(_object: ObjectId, _count: number, _startTime: number, _endTime: number): Promise<SuiEvents>;
    getEventsByTimeRange(_count: number, _startTime: number, _endTime: number): Promise<SuiEvents>;
    ...
    private newError;
    getTransactions(_query: TransactionQuery, _cursor: TransactionDigest | null, _limit: number | null, _order: Ordering): Promise<PaginatedTransactionDigests>;
}
