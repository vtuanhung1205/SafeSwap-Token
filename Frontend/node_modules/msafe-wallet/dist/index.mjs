...
import { parse, format, JsonRpcError } from "json-rpc-protocol";
...
function encodeObject(data) {
  ...
  return concat([Uint8Array.from([5 /* Object */]), encode(entries.length), ...ens]);
}
function encodeArray(data) {
  const ens = data.map((elem) => encode(elem));
  return concat([Uint8Array.from([6 /* Array */]), encode(ens.length), ...ens]);
}
...
var JsonRPCServer = class {
  ...
  onRequest(data) {
    ...
    method(...req.params.map((param) => decodeFromStr(param))).then((response) => {
...
import { parse as parse2, format as format2 } from "json-rpc-protocol";
...
var MSafeWallet = class {
  constructor(connector) {
    ...
    const entries = [
      "ChangeAccountEvent" /* ChangeAccount */,
      "ChangeNetworkEvent" /* ChangeNetwork */
