...
// --- Helper Hook for On-Scroll Animations (Unchanged) ---
...
// --- MODIFIED: Added coingeckoId to each token ---
const tokens = [
  {
    symbol: "BTC",
    name: "Bitcoin",
...
const SwapForm = () => {
  ...
  // --- ADDED: State for storing CoinGecko prices ---
  ...
  // --- REMOVED: WebSocket subscription useEffect ---
  // useEffect(() => {
  //   const symbols = tokens.map((token) => token.symbol);
  //   subscribeToTokens(symbols);
  // }, [subscribeToTokens]);

  // --- ADDED: useEffect to fetch prices from CoinGecko ---
  useEffect(() => {
    ...
    fetchTokenPrices(); // Fetch immediately on mount
  ...
  // --- Unchanged Functions: fetchWalletBalances, getSwapQuote, analyzeToken, handleSwap, etc. ---
      ...
      >
        ...
        <div className="relative group">
          <div className="bg-[#1c1c24] rounded-2xl p-6 border border-[#2a2a35] shadow-lg">
            ...
            {quote && (
              <div className="bg-[#111112] rounded-xl p-3 mb-4 border border-[#2a2a35]">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm text-white">
                    {(quote.fee * 100).toFixed(2)}% ({quote.fee.toFixed(6)} {fromToken.symbol})
                  </span>
                </div>
              </div>
            )}

            {/* Scam Analysis (Unchanged) */}
            {scamAnalysis && (
              <div className={`rounded-xl p-3 mb-4 border ${ scamAnalysis.isScam ? "bg-red-500/10 border-red-500/30 text-red-400" : scamAnalysis.riskScore > 50 ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-400" : "bg-green-500/10 border-green-500/30 text-green-400"}`}>
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <div className="font-medium mb-1">{scamAnalysis.isScam ? "High Risk Token" : scamAnalysis.riskScore > 50 ? "Medium Risk Token" : "Low Risk Token"}</div>
            ...
            {/* Connect Wallet / Swap Button (Unchanged) */}
            {!isWalletConnected ? (
              <WalletConnect />
            ) : (
              <button
              ...
              >
                {isSwapping ? (
                  <div className="flex items-center justify-center space-x-2"><Loader2 size={18} className="animate-spin" /><span>Swapping...</span></div>
                ) : isLoadingQuote ? (
                  <div className="flex items-center justify-center space-x-2"><Loader2 size={18} className="animate-spin" /><span>Getting Quote...</span></div>
                ) : !quote ? (
                  "Enter Amount"
                ) : (
                  "Swap Tokens"
                ...
                {tokens.map((token) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end">
                      {/* --- REPLACED PriceChange with CoinGeckoPriceDisplay --- */}
                      <CoinGeckoPriceDisplay
                        isLoading={isPriceLoading}
                        priceData={tokenPrices?.[token.coingeckoId]}
                      />
                      {isWalletConnected && tokenBalances[token.symbol] && (
                        <div className="text-xs text-gray-400 mt-1">
                          {formatBalance(getTokenBalance(token.symbol), token.symbol)}
                        </div>
                      )}
                    </div>
                  </button>
...
// --- ADDED: New component to display CoinGecko price data ---
const CoinGeckoPriceDisplay = ({ isLoading, priceData }) => {
  if (isLoading) {
    return <div className="h-10 w-20 bg-[#2a2a35] rounded animate-pulse"></div>;
  }
...
// --- REMOVED: Old PriceChange component ---
// const PriceChange = ({ symbol }) => { ... };

// --- ChevronDown component (Unchanged) ---
